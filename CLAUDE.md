...
npm run build             # tsdown build every package that has a build script
...
**Exported declarations need explicit types.** `.d.ts` files are generated by oxc
(`dts: { generator: 'oxc' }` in every `tsdown.config.ts`), which transforms each
file on its own with no type checker — so the build does not depend on the
TypeScript version. An exported function without a return type, or an exported
`const` without an annotation (`createContext<T>(…)` needs `: Context<T>`), fails
the build with TS9007 / TS9010. Do not "fix" that by switching the generator back
to `tsc` / `tsgo`: under TypeScript 7 that generator is experimental, and with a
package-local tsconfig it emits sibling packages' `.d.ts` files into
`packages/*/src`, where an `index.d.ts` shadows `index.ts`. Do not add
`isolatedDeclarations` to a tsconfig either — TypeScript 7.0's `tsc` panics on it.
...
runtime "React is not defined", not a compile error. vitest/tsdown honor the
