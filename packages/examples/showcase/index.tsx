...
const app = await render(<App scenes={SCENES} autopilot={autopilot} speed={speed} />, backend);
const quitting = new AbortController();
void app.waitUntilExit().then(() => quitting.abort());

if (!manual && fits) {
  do {
    await play(backend, FULL_SCRIPT, { speed, signal: quitting.signal, gate: autopilot.gate, onTimeout: 'continue' });
