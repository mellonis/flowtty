// Terminal display width (in cells) of text, à la POSIX wcwidth/wcswidth.
// flowtty's cell grid is monospace: most glyphs occupy 1 column, East Asian
// Wide/Fullwidth glyphs and most emoji occupy 2, and combining marks / zero-
// width formatters occupy 0. There is no deps-free standard for this, so we
...
// to strip here (an ESC byte simply counts as a width-0 control char).

type Range = readonly [number, number];

// Zero-width: combining marks + format/bidi controls + variation selectors.
// Markus Kuhn's `combining` table (wcwidth.c), widely copied verbatim.
...
const WIDE: readonly Range[] = [
  ...
  [0xff00, 0xff60], // Fullwidth Forms (stops before halfwidth katakana U+FF61)
...
function inRanges(cp: number, table: readonly Range[]): boolean {
  let lo = 0;
...
export function charWidth(cp: number): 0 | 1 | 2 {
  // C0 controls + DEL + C1 controls — non-printing, no advance.
