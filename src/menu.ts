...
export function Menu({ items, title, helpHint, onExit, onPage, children }: MenuProps) {
  ...
  function renderTopBar(): ReactNode {
    ...
    return createElement(Box, { key: '__topbar', flexDirection: 'row', width: '100%' },
      title != null && createElement(Box, { key: '__title', bold: true }, titleText),
      ...items.map((it, i) => {
  ...
  void helpHint;
  return createElement(Box, { flexDirection: 'column', width: '100%', height: '100%' },
    renderTopBar(),
    ...panels.map((_, i) => renderPanel(i)),
    // Page area below the bar. `inert={active}` mutes input dispatch into the
    // subtree while the menu is engaged — list cursors etc. won't fight menu
    // navigation. The Box itself still lays out and paints normally.
    children != null && createElement(Box, {
      key: '__page', flexGrow: 1, flexDirection: 'column', inert: active,
    }, children),
  );
}
